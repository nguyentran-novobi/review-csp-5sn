...
class AccountChartTemplate(models.Model):
    ...
    def generate_journals(self, acc_template_ref, company, journals_dict=None):
        ...
        customer_deposit_journal = self.env['account.journal'].search([('company_id', '=', company.id),
                                                                       ('type', '=', 'general'),
                                                                       ('code', '=', 'CDEP')])
        if customer_deposit_journal:
            company.customer_deposit_journal_id = customer_deposit_journal

        vendor_deposit_journal = self.env['account.journal'].search([('company_id', '=', company.id),
                                                                     ('type', '=', 'general'),
                                                                     ('code', '=', 'VDEP')])
        if vendor_deposit_journal:
            company.vendor_deposit_journal_id = vendor_deposit_journal
