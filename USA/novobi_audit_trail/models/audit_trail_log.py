...
#    Copyright (C) 2022 Novobi LLC (<http://novobi.com>)
#
##############################################################################
    ...
    def _compute_reference_and_name(self):
        for log in self:
            ...
            if log.res_id and log.model_id:
                record = self.env[log.model_id.model].sudo().browse([log.res_id])
            if log.parent_id and log.parent_model_id:
                parent_record = self.env[log.parent_model_id].sudo().browse([log.parent_id])
            ...
            if parent_record:
                ...
                log.parent_name = hasattr(parent_record, 'name') and parent_record.name or parent_record.name_get()[0][1]
            else:
