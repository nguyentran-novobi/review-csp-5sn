...
class AuditTrailRule(models.Model):
    ...
    def _check_unique_rule_per_model(self):
        for rule in self:
            rule_existed = self.env['audit.trail.rule'].search_count([('id','!=',rule.id),('model_id', '=', rule.model_id.id)])
            if rule_existed:
                raise ValidationError("A tracking model must have only one tracking rule.")
