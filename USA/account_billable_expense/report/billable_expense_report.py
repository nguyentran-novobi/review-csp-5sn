...
class BillableExpenseReport(models.AbstractModel):
    ...
    def _get_templates(self):
        ...
        templates[
            "line_template"
        ] = "account_billable_expense.line_template_billable_expense_report"
        templates[
            "main_template"
        ] = "account_billable_expense.template_billable_expense_report"
        templates[
            "search_template"
        ] = "account_billable_expense.search_template_expense"
        return templates
    ...
    def group_by_partner_id(self, options, line_id):
        ...
        for partner in customer_ids:
            outstanding_expenses = partner.get_outstanding_expenses(
                options, company_ids
            )
            if not outstanding_expenses:
                continue

            for line in outstanding_expenses:
                ...
                partners[currency][partner]["amount"] = amount + line.amount_total
                partners[currency][partner]["lines"].append(line)
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for currency in partners:
            ...
            for partner in sorted_partners:
                ...
                partner_str = "partner_" + str(partner.id) + "_currency_" + str(currency.id)
                lines.append(
                    {
                        "id": partner_str,
                        "name": partner.name,
                        ...
                        "columns": [
                            {"name": v}
                            for v in [self.format_value(amount, currency=currency), ""]
                        ],
                        "unfoldable": True,
                        "unfolded": partner_str in options.get("unfolded_lines")
                        or unfold_all,
                        "level": 2,
                        "colspan": 5,
                    }
                        ...
                        domain_lines.append(
                            {
                                ...
                                "columns": [
                                    type(v) == dict and v or {"name": v}
                                    for v in columns
                                ],
                                "level": 3,
                                "caret_options": "billable.expenses"
                                if line.bill_id
                                else "purchase.expenses",
                            }
                        )
                    lines += domain_lines
                ...
                lines.append(
                    {
                        ...
                        "columns": [
                            {"name": v}
                            for v in ["", "", "", "", self.format_value(total_amount, currency=currency), ""]
                        ],
                    }
                )
            ...
            lines.append(
                {
                    ...
                    "unfoldable": False,
                    "level": 0,
                    "columns": [{"name": ''} for _ in range(0,6)],
                }
            )
        return lines
    ...
    def open_bill_expense(self, options, params=None):
        ...
        if expense_id:
            ...
            view_id = self.env["ir.model.data"]._xmlid_lookup("%s.%s" % ('account', 'view_move_form'))[1:3][1]
            return {
