...
class PartialPayment(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        if move_line_id.currency_id and move_line_id.currency_id == invoice_currency:
            payment_amount = abs(move_line_id.amount_residual_currency)
        else:
            currency = move_line_id.company_id.currency_id
            payment_amount = currency._convert(abs(move_line_id.amount_residual), invoice_currency,
                                               invoice_id.company_id, move_line_id.date or fields.Date.today())

        res['amount'] = invoice_currency.compare_amounts(amount_due, payment_amount) == -1 and amount_due or payment_amount
        return res
    ...
    def apply(self):
        if not self.have_same_currency:  # we don't need to validate the amount if they have different currency
            ...
            if self.currency_id.compare_amounts(self.amount, 0) <= 0:
                raise UserError(_('You entered an invalid value. Please make sure you enter a value is greater than 0.'))
            elif self.amount > abs(self.move_line_id.amount_residual_currency) or self.amount > self.invoice_id.amount_residual:
                raise UserError(_('You entered a value that exceeds the amount due/outstanding amount of the transactions'))
