...
class BankReconciliationDifference(models.TransientModel):
    ...
    company_id = fields.Many2one('res.company', string='Company', required=True,
                                 default=lambda self: self.bank_reconciliation_data_id.company_id)
    ...
    def _get_account_side(self, journal_id, difference):
        ...
        company_currency = self.bank_reconciliation_data_id.currency_id
    ...
    def apply(self):
        ...
        journal = self.bank_reconciliation_data_id.journal_id
        # There are two cases:
        # 1. Journal has foreign currency and bank reconciliation session is in company currency
        # 2. Journal has foreign currency and bank reconciliation session is in foreign currency
        company_currency = journal.company_id.currency_id
        currency = journal.currency_id or company_currency
        reconcile_currency = self.bank_reconciliation_data_id.currency_id

        debit_account, credit_account = self._get_account_side(journal, difference)

        # Convert difference into company currency when bank reconciliation session is in foreign currency
        if reconcile_currency != journal.company_id.currency_id:
            difference = reconcile_currency._convert(
                difference, company_currency, journal.company_id, self.adjustment_date or fields.Date.today())
