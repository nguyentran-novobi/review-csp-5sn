...
class AccountRecordEndingBalance(models.TransientModel):
    ...
    currency_id = fields.Many2one('res.currency', readonly=True, related='bank_reconciliation_data_id.currency_id')
    ...
    def apply(self):
        ...
        if not journal_id.company_id.account_journal_payment_debit_account_id:
        ...
        bill_id = self.env['account.move'].sudo().create({
            ...
            'invoice_line_ids': [Command.create({
                ...
                'account_id': journal_id.company_id.account_journal_payment_debit_account_id.id,
                'quantity': 1,
                'price_unit': balance
            })]
        })
        ...
        if options == 'create_purchase_receipt':
            ...
            payment_id = self.env['account.payment'].sudo().create({
                ...
                'payment_method_line_id': journal_id.outbound_payment_method_line_ids and journal_id.outbound_payment_method_line_ids[0].id or False,
                'date': today,
