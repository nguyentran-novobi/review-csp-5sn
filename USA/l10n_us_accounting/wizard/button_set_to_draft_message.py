...
class AccountPaymentToDraftPopup(models.TransientModel):
    ...
    def _get_confirmed_message_payment(self, payment):
        ...
        if payment and payment.state != 'draft':
            ...
            if float_compare(payment.applied_amount, 0, precision_rounding=payment.currency_id.rounding) > 0:
                message = "This payment has been applied{}. Resetting it to draft will remove it from all related invoices/bills"
                
            line_ids = payment.move_id.line_ids.filtered(
                lambda line: line.account_id.id in payment.journal_id.get_all_journals_outstanding_accounts().mapped('id'))
    ...
    def _get_confirmed_message_move(self, move):
        ...
        if move and move.state != 'draft':
            line_ids = move.line_ids.filtered(lambda line: line.account_id.id in line.journal_id.get_all_journals_outstanding_accounts().mapped('id'))
    ...
    def button_set_to_draft(self):
        ...
        if self.payment_id:
            ...
            outstanding_account_ids = journal_id.get_all_journals_outstanding_accounts().mapped('id')
