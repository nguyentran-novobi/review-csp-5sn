...
class BankReconciliationData(models.Model):
    ...
    start_date = fields.Date('Start Date', related='journal_id.show_transactions_from', readonly=False, store=True)
    ...
    def default_get(self, fields):
        ...
        if defaults.get('journal_id', False):
            ...
            if previous_reconciliation:
                ...
                defaults['currency_id'] = previous_reconciliation.currency_id.id
            elif journal_id and journal_id.currency_id:
                defaults['currency_id'] = journal_id.currency_id.id
            elif journal_id:
                defaults['currency_id'] = journal_id.company_id.currency_id.id
    ...
    def _get_transactions(self, old_date=None, new_date=None):
        ...
        + OR (outstanding accounts & payment_id & reconciled): journal type is different bank and cash
        ...
        + OR (bank account & not statement_line_id): for manual journal entry that goes directly to Bank
        """
        ...
        outstanding_account_ids = journal_id.get_all_journals_outstanding_accounts().ids
        exclude_account_ids = [bank_account_id, journal_id.suspense_account_id.id, *outstanding_account_ids]

        sql = """
            SELECT aml.id
            FROM account_move_line AS aml
                JOIN account_move AS am ON aml.move_id = am.id
                JOIN account_journal AS aj ON aml.journal_id = aj.id
            WHERE aml.date <= %(statement_ending_date)s {date_condition}
                AND aml.bank_reconciled IS NOT TRUE
                AND am.state = 'posted'
                AND (
                    (
                        aml.journal_id = %(journal_id)s 
                        AND (
                            aml.account_id IN %(outstanding_account_ids)s AND aml.statement_line_id IS NULL AND aml.reconciled = TRUE
                            OR (aml.account_id IN %(outstanding_account_ids)s AND aml.batch_fund_line_id IS NOT NULL AND aml.reconciled = TRUE)
                            OR (aml.account_id NOT IN %(exclude_account_ids)s AND aml.statement_line_id IS NOT NULL)
                        )
                    )
                    OR (
                        aml.account_id IN %(outstanding_account_ids)s AND aml.statement_line_id IS NULL AND aml.reconciled = TRUE
                    ...
                    OR (aml.account_id = %(bank_account_id)s AND aml.statement_line_id IS NULL)
                )
        """
        ...
        if old_date:
            if self.start_date and old_date < self.start_date:
                date_condition = 'AND aml.date >= %(start_date)s'
            else:
                date_condition = 'AND aml.date > %(old_date)s'
        ...
        params = {
            ...
            'exclude_account_ids': tuple(exclude_account_ids),
    ...
    @api.constrains('start_date', 'statement_ending_date')
    def _check_start_date(self):
        for record in self:
            if record.start_date and record.state == 'draft' and record.start_date > record.statement_ending_date:
                raise ValidationError(
                    _('Show transactions from must be less than or equal to Statement Ending Date'))
    ...
    def undo_last_reconciliation(self):
        if not self.previous_reconciliation_id:
            raise UserError(_('There are no previous reconciliations to undo.'))
    ...
    def check_difference_amount(self, aml_ids, difference, cleared_payments, cleared_deposits):
        ...
        if float_compare(difference, 0.0, precision_digits=2) != 0:  # difference != 0
            formatted_value = formatLang(self.env, 0.0, currency_obj=self.currency_id)
            return {
                'name': "Difference Amount isn't balance ({}) yet".format(formatted_value),
