...
class AccountPaymentUSA(models.Model):
    ...
    def _compute_entries_amount(self):
        for record in self:
            ...
            record.outstanding_payment = abs(
                sum((counterpart_lines.mapped('amount_residual_currency')))) if counterpart_lines else 0
    ...
    def action_post(self):
        ...
        for payment in self:
            # Amount Validation
            if payment.currency_id.compare_amounts(payment.amount, payment.to_apply_amount) < 0:
            ...
            for open_tran in payment.payment_line_ids:
                ...
                for counterpart in counterpart_lines:
                    ...
                    tran_aml.move_id.with_context(partial_amount=open_tran.payment).js_assign_outstanding_line(
                        counterpart.id)
    ...
    def _synchronize_to_moves(self, changed_fields):
        ...
        if 'is_payment_receipt' in changed_fields and not payment_receipts:
            # When change from payment receipt to normal payment, we need to remove write-off move line if amount is not zero
            for payment in self.with_context(skip_account_move_synchronization=True):
                ...
                # If there are 3 lines in journal entry, remove write-off move line
                if len(line_vals_list) > 2 or other_lines:
                    line_ids_commands = [
                        Command.update(liquidity_lines.id, line_vals_list[0]),
                        Command.update(counterpart_lines.id, line_vals_list[1]),
                        Command.delete(other_lines[0].id) 
                    ]
                else:
                    line_ids_commands = [
                        Command.update(liquidity_lines.id, line_vals_list[0]),
                ...
                payment.move_id.with_context(from_payment_receipt=True).write({
            ...
            for payment in payment_receipts:
                ...
                if other_lines:
                    # Just update write-off move line if amount is changed, delete write-off move line if amount is zero
                    line_ids_commands = [
                        Command.update(liquidity_lines.id, line_vals_list[0]),
                        Command.update(counterpart_lines.id, line_vals_list[1]),
                        Command.update(other_lines[0].id, line_vals_list[2]) if len(line_vals_list) > 2 else Command.delete(other_lines[0].id),
                    ]
                else:
                    # Need to set debit and credit of counter part move line to 0
                    # Create write-off move line if amount is not zero
                    line_vals_list[1].update(debit=0, credit=0)
                    if len(line_vals_list) > 2:
                        line_ids_commands = [
                            Command.update(liquidity_lines.id, line_vals_list[0]),
                            Command.update(counterpart_lines.id, line_vals_list[1]),
                            Command.create(line_vals_list[2])
                        ]
                    else:
                        line_ids_commands = [
                            Command.update(liquidity_lines.id, line_vals_list[0]),
