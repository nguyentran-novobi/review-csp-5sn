...
class USABankReconciliation(models.AbstractModel):
    ...
    def get_html(self, options, line_id=None, additional_context=None):
        ...
        additional_context.update({
            ...
            'formatted_beginning': self.format_value(beginning_balance, bank_reconciliation_data_id.currency_id),
            'formatted_ending': self.format_value(ending_balance, bank_reconciliation_data_id.currency_id),
    ...
    def _get_lines(self, options, line_id=None):
        ...
        lines = []
        bank_reconciliation_data_id = self._get_bank_reconciliation_data_id()
        company = bank_reconciliation_data_id.company_id
        reconcile_currency = bank_reconciliation_data_id.currency_id
        company_currency = company.currency_id

        aml_ids = bank_reconciliation_data_id._get_transactions()
        bank_reconciliation_data_id.write({'aml_ids': [Command.set(aml_ids.ids)]})
        ...
        for line in aml_ids:
            ...
            # Convert amount into journal currency
            if reconcile_currency != company_currency:
                credit = company_currency._convert(
                    credit, reconcile_currency, company, line.date or fields.Date.today())
                debit = company_currency._convert(
                    debit, reconcile_currency, company, line.date or fields.Date.today())
            ...
            columns = [self._format_date(line.date),
                       ...
                       line.name,
                       check_number,
                       self.format_value(credit, bank_reconciliation_data_id.currency_id) if credit > 0 else '',
                       self.format_value(debit, bank_reconciliation_data_id.currency_id) if debit > 0 else '',
