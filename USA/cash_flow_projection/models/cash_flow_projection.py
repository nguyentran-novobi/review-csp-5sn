...
class CashFlowProjection(models.TransientModel):
    ...
    def _query_liquidity_lines(self, options):
        ...
        query = """
            ...
            WHERE account.internal_type = 'liquidity' 
                AND account_move_line.date <= '{}' 
    ...
    def _query_cash_in_lines(self, from_date, to_date, options={}):
        ...
        query_stmt = """
            SELECT temp.id as transaction_code, 
                temp.name as transaction_name, 
                ROUND(CAST(SUM(temp.amount) AS numeric),2) as amount
            FROM ({}) temp
    ...
    def _query_incoming_payment_lines(self, from_date, to_date, options):
        ...
        query_incoming_payment_lines = """
            SELECT cast('future_customer_payment' as text) as id, 
                cast('Future Customer Payments' as text) as name,
                CASE
            ...
            WHERE am.state = 'posted'
                ...
                AND (aml.date + aa.payment_lead_time) <= '{}'
                AND (aml.account_id = company.account_journal_payment_debit_account_id
                    OR aml.account_id IN (SELECT apml.payment_account_id
                        FROM account_payment_method_line apml 
                            ...
                            WHERE apml.journal_id = aj.id AND apm.payment_type = 'inbound'))
    ...
    def _query_so_lines(self, from_date, to_date, options):
        ...
        query_so_lines = """
            SELECT cast('sale_order' as text) as id, 
                cast('Sales' as text) as name,
                CASE
                ...
                END AS amount,
                so.name as account_name, 
                TO_CHAR(so.date_order + interval '{}' day, 'mm/dd/yyyy') as date, 
                so.id as line_id, 
                so.id as account_id, 
                rp.name as partner_name
            FROM sale_order so 
    ...
    def _query_ar_invoice_lines(self, from_date, to_date, options):
        ...
        query_ar_invoice_lines = """
            SELECT cast('ar_invoice' as text) as id, 
                cast('Receivable' as text) as name, 
                CASE
                ...
                aml1.partner_name
            FROM
                (SELECT aml.amount_residual, aml.amount_residual_currency, aml.currency_id, aml.move_id, aml.date_maturity, 
                        aml.id, aa.id as account_id, rp.name as partner_name
    ...
    def _query_ar_credit_note_lines(self, from_date, to_date, options):
        ...
        query_ar_credit_note_lines = """
            SELECT cast('ar_credit_note' as text) as id, 
                cast('Customer Credit Notes' as text) as name, 
                CASE
                    WHEN aml1.currency_id != {} THEN ROUND(aml1.amount_residual * currency_table.rate, currency_table.precision)
                    ELSE aml1.amount_residual_currency
                END AS amount,
                    ...
                    AND am.id NOT IN (
                        ...
                        WHERE am.state = 'posted'
                            ...
                            AND (aat.type = 'liquidity'
                                OR aml.account_id = company.account_journal_payment_debit_account_id
                                OR aml.account_id IN (SELECT apml.payment_account_id
                                    FROM account_payment_method_line apml 
                                        JOIN account_payment_method apm ON apml.payment_method_id = apm.id
                                        WHERE apml.journal_id = aj.id AND apm.payment_type = 'inbound')
    ...
    def _query_outgoing_payment_lines(self, from_date, to_date, options):
        ...
        query_outgoing_payment_lines = """
            SELECT cast('future_vendor_payment' as text) as id, 
                cast('Future Vendor Payments' as text) as name, 
                CASE
            ...
            WHERE am.state = 'posted'
                ...
                AND (aml.date + aa.payment_lead_time) >= '{}'
                AND (aml.date + aa.payment_lead_time) <= '{}'
                AND (aml.account_id = company.account_journal_payment_credit_account_id
                    OR aml.account_id IN (SELECT apml.payment_account_id
                        FROM account_payment_method_line apml 
                            JOIN account_payment_method apm ON apml.payment_method_id = apm.id
                            WHERE apml.journal_id = aj.id AND apm.payment_type = 'outbound'))
                AND aml.company_id IN {}
        """.format(options['main_company_currency_id'], options['currency_table_query'], from_date, to_date,
                   options['companies'])
    ...
    def _query_po_lines(self, from_date, to_date, options):
        ...
        query_po_lines = """
            SELECT cast('purchase_order' as text) as id, 
                cast('Purchases' as text) as name, 
                CASE
                ...
                END AS amount,
                po.name as account_name, 
                TO_CHAR(po.date_approve + interval '{}' day, 'mm/dd/yyyy') as date, 
                po.id as line_id, 
                po.id as account_id, 
                rp.name as partner_name
    ...
    def _query_ap_credit_note_lines(self, from_date, to_date, options):
        ...
        query_ap_credit_note_lines = """
            SELECT cast('ap_credit_note' as text) as id, 
                cast('Vendor Credit Notes' as text) as name, 
                CASE
                    WHEN aml1.currency_id != {} THEN ROUND(-aml1.amount_residual * currency_table.rate, currency_table.precision)
                    ELSE -aml1.amount_residual_currency
                END AS amount, 
            ...
            WHERE am.state = 'posted'
                AND am.id NOT IN (
                    ...
                    WHERE am.state = 'posted'
                        AND aml.credit > 0
                        AND (aat.type = 'liquidity'
                            OR aml.account_id = company.account_journal_payment_credit_account_id
                            OR aml.account_id IN (SELECT apml.payment_account_id
                                FROM account_payment_method_line apml 
                                    JOIN account_payment_method apm ON apml.payment_method_id = apm.id
                                    WHERE apml.journal_id = aj.id AND apm.payment_type = 'outbound')
